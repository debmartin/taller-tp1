...
bool ControladorPersonaje::manejar_Evento(SDL_Event &evento){
    ...
    if(tipoDeControl == "JOYSTICK"){
    	...
    	if(modo_juego == "P1_vs_P2"){
    		if(!personaje2->estaEnCaida() && !personaje2->estaBloqueado() && !personaje2->estaMareado() && !personaje2->estaSinEnergia() && !personaje2->haciendoFatality() && !personaje2->recibioFatality() && !personaje2->estaEnFatality()){
...
void ControladorPersonaje::identificarOrdenJoystickPersonaje(Personaje* personaje, JoyNumber numeroJoystick){
	...
	//TheInputHandler::Instance()->imprimirColaBotones(numeroJoystick);
